...
# @testset "test_psid_paper" begin
    base_path = (joinpath(pwd(), "test_psid_paper"))
    !isdir(base_path) && mkdir(base_path)
   # try 
        # 1. Build the system in PSID. 
        sys = System(joinpath(@__DIR__, "systems_tests", "psid_paper_system.json"), runchecks = false)
        ...
        if plotting 
            p = plot_psid_pscad_initialization_comparison(sys, psid_results, pscad_results)
            display(p)
        end 
        psid_pscad_initialization_comparison(sys, psid_results, pscad_results)
        project.save()   
        pscad.save_workspace()
...
#=     finally
        ...
        rm(base_path, force = true, recursive = true)
    end 
end 
 =#
